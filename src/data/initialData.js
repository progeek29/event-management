// Shree Ram Events — Curated Master Data from Authentic Brand Standee

export const BRAND_INFO = {
  name: "Shree Ram Events",
  wordmark: "SHREE RAM",
  subtitle: "CATERING • EVENT PLANNING • DECORATION",
  tagline: "Bespoke Celebrations Fit for Royalty",
  subTagline: "Crafting monumental Indian weddings, private soirées, and 5-star royal banquets across Chhattisgarh.",
  sacredInvocation: "॥ श्री गणेशाय नमः ॥",
  location: "Bhilai · Raipur · Durg · Chhattisgarh",

  // Real team & phone numbers from banner standee
  contacts: [
    {
      name: "Vishal Pratap Singh",
      role: "Founder & Creative Director",
      phone: "7000342216",
      altPhone: "7067242505",
      phoneDisplay: "+91 70003 42216",
      whatsapp: "917000342216",
    },
    {
      name: "Amitab Verma",
      role: "Co-Founder & Culinary Lead",
      phone: "8305947913",
      phoneDisplay: "+91 83059 47913",
      whatsapp: "918305947913",
    },
    {
      name: "Chhatrapal",
      role: "Operations & On-Ground Director",
      phone: "9589137935",
      phoneDisplay: "+91 95891 37935",
      whatsapp: "919589137935",
    }
  ],
  instagram: {
    handle: "@shreeramevents_bhilai",
    url: "https://www.instagram.com/shreeramevents_bhilai/",
  },
  stats: [
    { number: "500+", label: "Royal Weddings" },
    { number: "100%", label: "Taste & Purity" },
    { number: "10+", label: "Years of Pedigree" },
    { number: "4.9★", label: "Client Prestige" },
  ]
};

// Curated 5 Core Services from Brand Standee
export const BANNER_SERVICES = [
  {
    id: "weddings",
    number: "01",
    title: "Destination Weddings & Mandaps",
    titleHi: "शाही विवाह एवं मंडप समारोह",
    subtitle: "Royal Mandaps, Varmala & Sangeet",
    subtitleHi: "भव्य मंडप, वरमाला स्टेज व संगीत",
    description: "End-to-end bespoke wedding architecture. From monumental glass and hand-carved mandaps to celebrity baraat entries, revolving hydraulic varmala stages, and intelligent concert sound.",
    descriptionHi: "सम्पूर्ण शाही विवाह प्रबंधन — नक्काशीदार भव्य मंडप, सेलिब्रिटी स्टाइल बारात एंट्री, हाइड्रोलिक वरमाला स्टेज, और विश्वस्तरीय साउंड व लाइट व्यवस्था।",
    image: "/events/moments-become-memories.jpg",
    features: [
      "Custom Carved Mandaps & Glass Pavilions",
      "Celebrity-Style Royal Bride & Groom Grand Entry",
      "Cold Pyro & Theatrical Atmosphere Special FX",
      "Dedicated Event Captain for Zero-Stress Hosting"
    ],
    featuresHi: [
      "राजसी नक्काशीदार मंडप एवं ग्लास पवेलियन",
      "रॉयल ब्राइड एवं ग्रूम भव्य एंट्री",
      "कोल्ड पायरो एवं थियेट्रिकल स्पेशल इफेक्ट्स",
      "तनावमुक्त आयोजन हेतु समर्पित इवेंट कैप्टन"
    ]
  },
  {
    id: "engagement",
    number: "02",
    title: "Engagement & Sangeet Nights",
    titleHi: "सगाई, संगीत एवं भव्य रिसेप्शन",
    subtitle: "Opulent Stages, Florals & Grand Banquets",
    subtitleHi: "आकर्षक स्टेज, फ्लोरल डेकोर व बैंक्वेट",
    description: "Enchanting reception stages adorned with fresh imported florals, vintage crystal chandeliers, cascading light tunnels, and curated fine dining for high-society gatherings.",
    descriptionHi: "ताजे विदेशी फूलों से सुसज्जित भव्य स्टेज, विंटेज क्रिस्टल झूमर, आकर्षक लाइट टनल, और उच्च स्तरीय डाइनिंग अनुभव।",
    image: "/events/unplanned-moments.jpg",
    features: [
      "Designer 40ft Luxury Stage Facades & Arches",
      "Live Acoustic Symphony & Musician Curation",
      "Memory Photo Pathway with Amber Fairy Lights",
      "Brass & Porcelain Plated VIP Guest Service"
    ],
    featuresHi: [
      "40 फीट लक्ज़री डिजाइनर स्टेज व मेहराब",
      "लाइव एकॉस्टिक सिम्फनी एवं म्यूजिशियन",
      "मेमोरी फोटो पाथवे विथ एम्बर लाइट्स",
      "ब्रास एवं क्रॉकरी प्लेटेड वीआईपी हॉस्पिटैलिटी"
    ]
  },
  {
    id: "catering",
    number: "03",
    title: "Catering & Royal Feasts",
    titleHi: "शाही कैटरिंग एवं लज़ीज़ दावत",
    subtitle: "5-Star Awadhi, Live Chaat & Global Menus",
    subtitleHi: "शुद्ध घी व्यंजन, लाइव चाट व 5-स्टार मेनू",
    description: "Pure-ghee traditional delicacies, live Banarasi chaat bazzar, wood-fired continental stations, and artisanal royal mithai prepared by seasoned Awadhi master chefs.",
    descriptionHi: "शुद्ध देशी घी के पारंपरिक पकवान, लाइव बनारसी चाट बाज़ार, कॉन्टिनेंटल फ़ूड स्टेशन, और अवधी उस्ताद शेफ द्वारा तैयार शाही मिठाइयाँ।",
    image: "https://images.unsplash.com/photo-1555244162-803834f70033?q=80&w=1200&auto=format&fit=crop",
    features: [
      "Authentic Traditional Pure-Ghee Sweets & Desserts",
      "Live Delhi-6 & Banarasi Chaat Street Counters",
      "Traditional Awadhi, Mughlai & South Indian Buffets",
      "100% Hygiene Audited Uniformed Catering Staff"
    ],
    featuresHi: [
      "पारंपरिक शुद्ध देशी घी की मिठाइयाँ व डिज़र्ट्स",
      "लाइव दिल्ली-6 एवं बनारसी चाट स्ट्रीट काउंटर्स",
      "अवधी, मुग़लई व दक्षिण भारतीय भव्य बुफे",
      "100% हाइजीनिक एवं यूनिफ़ॉर्म्ड कैटरिंग स्टाफ"
    ]
  },
  {
    id: "corporate",
    number: "04",
    title: "Sprawling Lawns & Estates",
    titleHi: "भव्य लॉन, वेन्यू एवं थीम उत्सव",
    subtitle: "Open Lawns, Poolside Haldi & Banquets",
    subtitleHi: "ओपन लॉन, पूलसाइड हल्दी व बैंक्वेट",
    description: "Lush expansive green venues for outdoor pheras, cocktail evenings, poolside haldi ceremonies, and luxury guest hospitality with 100+ room accommodations.",
    descriptionHi: "ओपन एयर फेरे, कॉकटेल नाइट्स, पूलसाइड हल्दी और भव्य पारिवारिक सम्मेलनों हेतु विशाल लॉन व बैंक्वेट सुविधाएं।",
    image: "/events/laughter-chaos-love.jpg",
    features: [
      "40,000+ Sq.Ft Landscaped Lawns & Gazebos",
      "Poolside Mehendi & Vibrant Haldi Setups",
      "Concert Lighting Truss & Synchronized Audio FX",
      "VIP Hospitality Concierge & Guest Escort Fleet"
    ],
    featuresHi: [
      "40,000+ वर्गफीट हरा-भरा लैंडस्केप लॉन",
      "पूलसाइड मेहंदी एवं हल्दी डेकोर",
      "सिंक्रोनाइज़्ड कॉन्सर्ट लाइट व साउंड",
      "वीआईपी हॉस्पिटैलिटी एवं गेस्ट एस्कॉर्ट"
    ]
  },
  {
    id: "housewarming",
    number: "05",
    title: "Housewarming & Sacred Rituals",
    titleHi: "गृह प्रवेश एवं धार्मिक अनुष्ठान",
    subtitle: "Griha Pravesh, Ramotsav & Bhajan Sandhya",
    subtitleHi: "गृह प्रवेश, रामोत्सव व भजन संध्या",
    description: "Auspicious temple-style pandals, brass diya illuminations, fresh marigold and jasmine installations, accompanied by pure satvik prasadam catering.",
    descriptionHi: "पवित्र मंदिर शैली पंडाल, पीतल दीयों की जगमगाहट, गेंदे व चमेली फूलों का भव्य श्रृंगार, एवं 100% शुद्ध सात्विक प्रसादम कैटरिंग।",
    image: "/src/assets/housewarming_event.jpg",
    features: [
      "Sacred Pandal Decor & Fresh Jasmine Singhasan",
      "108 Brass Deepam Illumination & Diya Setups",
      "100% Pure Satvik Prasadam (No Onion / Garlic)",
      "Devotional Sound & Traditional Seating Arrangement"
    ],
    featuresHi: [
      "पवित्र पंडाल डेकोर एवं ताजे फूलों का सिंहासन",
      "108 पीतल दीप प्रज्वलन व भव्य दीया सज्जा",
      "100% शुद्ध सात्विक प्रसादम (बिना प्याज-लहसुन)",
      "भक्ति संगीत साउंड एवं पारंपरिक बैठक व्यवस्था"
    ]
  }
];

// Interactive Showcase Portfolio
export const SHOWCASE_PORTFOLIO = [
  {
    id: "show-1",
    title: "The Sacred Vows Ceremony",
    location: "Royal Courtyard Lawns",
    theme: "Heritage Ivory & Brass",
    guestSize: "850 Guests",
    category: "Weddings",
    image: "/events/moments-become-memories.jpg",
    details: "A dramatic outdoor floral mandap framed by hand-carved pillars, cascading jasmine canopies, and radiant couple walk."
  },
  {
    id: "show-2",
    title: "The Unscripted Joy",
    location: "Palace Garden Gazebo",
    theme: "Peach Silk & Sunset Florals",
    guestSize: "600 Guests",
    category: "Weddings",
    image: "/events/unplanned-moments.jpg",
    details: "Spontaneous bridal twirl under flower showers, capturing the authentic emotion of the wedding morning."
  },
  {
    id: "show-3",
    title: "The Living Celebration",
    location: "Grand Heritage Lawns",
    theme: "Mughal Mirror & Candlelight",
    guestSize: "1,100 Guests",
    category: "Engagement",
    image: "/events/laughter-chaos-love.jpg",
    details: "Surrounded by clapping families, flower petal rains, and orchestral acoustic melodies under the stars."
  },
  {
    id: "show-4",
    title: "Every Sacred Detail",
    location: "Private Bridal Pavilion",
    theme: "Ancestral Polki & Henna",
    guestSize: "Intimate Atelier",
    category: "Parties",
    image: "/events/every-detail-matters.jpg",
    details: "Intricate bridal mehendi, heritage kadas, polki rings, and hand-embroidered lehenga craftsmanship."
  },
  {
    id: "show-5",
    title: "The Cinematic Narrative",
    location: "Estates of Chhattisgarh",
    theme: "Editorial Photography & Films",
    guestSize: "Full Celebration",
    category: "Weddings",
    image: "/events/timeless-stories-editorial.jpg",
    details: "Candid, cinematic, and emotional documentation preserving memories that will outlive generations."
  },
  {
    id: "show-6",
    title: "Artisanal Awadhi Feast",
    location: "Nehru Nagar, Bhilai",
    theme: "Royal Culinary Feasts",
    guestSize: "750 Guests",
    category: "Catering",
    image: "https://images.unsplash.com/photo-1555244162-803834f70033?q=80&w=1200&auto=format&fit=crop",
    details: "Live Banarasi chaat counters, kulhad kesar chai, and pure-ghee royal desserts prepared fresh on site."
  }
];

// Initial Royal Farmaan Inquiries
export const INITIAL_LEADS = [
  {
    id: "SRE-2026-001",
    dateSubmitted: "18 Sep 2026, 02:30 PM",
    clientName: "Khurana Family",
    phone: "9826198765",
    occasion: "Royal Wedding & Mandap Setup",
    eventDate: "2026-11-28",
    guestCount: "600 - 1,500+ Guests (Sprawling Lawn Gala)",
    city: "Bhilai / Durg",
    status: "Date Confirmed",
    notes: "Heritage Ivory & Brass mandap. Awadhi royal banquet and live chaat street requested."
  },
  {
    id: "SRE-2026-002",
    dateSubmitted: "18 Sep 2026, 05:15 PM",
    clientName: "Pooja Agrawal & Family",
    phone: "9425212345",
    occasion: "Engagement & Sangeet Gala",
    eventDate: "2026-12-04",
    guestCount: "300 - 600 Guests (Grand Royal Banquet)",
    city: "Raipur",
    status: "Founder Contacted",
    notes: "Silver jubilee gala. Intimate candlelit cabana and live royal acoustic troupe."
  },
  {
    id: "SRE-2026-003",
    dateSubmitted: "19 Sep 2026, 09:45 AM",
    clientName: "Vikram Singhania",
    phone: "9755567890",
    occasion: "Corporate & Theme Galas",
    eventDate: "2026-10-18",
    guestCount: "300 - 600 Guests (Grand Royal Banquet)",
    city: "Bhilai / Durg",
    status: "Farmaan Bestowed",
    notes: "Annual corporate gala. Requires majestic stage AV setup and 5-star executive banquet."
  },
  {
    id: "SRE-2026-004",
    dateSubmitted: "19 Sep 2026, 11:05 AM",
    clientName: "Ananya Mishra & Family",
    phone: "8982054321",
    occasion: "Royal Wedding & Mandap Setup",
    eventDate: "2026-12-19",
    guestCount: "600 - 1,500+ Guests (Sprawling Lawn Gala)",
    city: "Raipur",
    status: "Farmaan Bestowed",
    notes: "Full royal wedding production, regal floral mandap, and grand bridal entrance in Raipur."
  }
];

// Initial Confidential Financial Ledger
export const INITIAL_FINANCIALS = [
  {
    id: "FIN-2026-001",
    leadId: "SRE-2026-001",
    clientName: "Rajeshwar Sharma",
    eventName: "Royal Palace Wedding (3 Days)",
    contractValue: 750000,
    advancePaid: 350000,
    balanceDue: 400000,
    decorExpense: 220000,
    cateringExpense: 260000,
    otherExpense: 35000,
    paymentStatus: "Partial (Advance Received)",
    dueDate: "2026-11-20"
  },
  {
    id: "FIN-2026-002",
    leadId: "SRE-2026-002",
    clientName: "Pooja Agrawal",
    eventName: "25th Silver Jubilee Soirée",
    contractValue: 240000,
    advancePaid: 120000,
    balanceDue: 120000,
    decorExpense: 65000,
    cateringExpense: 85000,
    otherExpense: 12000,
    paymentStatus: "Partial (Advance Received)",
    dueDate: "2026-11-25"
  },
  {
    id: "FIN-2026-003",
    leadId: "SRE-2026-003",
    clientName: "Vikram Singhania",
    eventName: "Annual Corporate Gala Dinner",
    contractValue: 380000,
    advancePaid: 0,
    balanceDue: 380000,
    decorExpense: 110000,
    cateringExpense: 135000,
    otherExpense: 20000,
    paymentStatus: "Invoice Sent",
    dueDate: "2026-10-10"
  }
];
